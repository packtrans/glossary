...
mod tests {
    ...
    /// Languages whose tokenizer is "default" should not require any dictionary.
    #[test]
    fn ensure_tokenizer_dictionary_is_noop_for_default_lang() {
        // en_us maps to the default tokenizer — no dictionary dir needed.
        let result = ensure_tokenizer_dictionary("en_us", None);
        assert!(result.is_ok());
    }

    #[test]
    fn ensure_tokenizer_dictionary_is_noop_for_de_de() {
        let result = ensure_tokenizer_dictionary("de_de", None);
        assert!(result.is_ok());
    }

    /// When the dictionary directory already exists on disk the function should
    /// return Ok without attempting a network download.
    #[test]
    fn ensure_tokenizer_dictionary_ok_when_dict_dir_exists() {
        let root = temp_root("dict-exists");
        let version = packtrans_glossary_core::dictionary::current_version();
        // Create the expected jieba dictionary directory.
        let dict_dir = root
            .join(version)
            .join(packtrans_glossary_core::dictionary::JIEBA);
        fs::create_dir_all(&dict_dir).unwrap();

        // Should succeed without touching the network.
        let result = ensure_tokenizer_dictionary("zh_cn", Some(&root));
